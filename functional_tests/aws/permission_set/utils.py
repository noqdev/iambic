...
from iambic.aws.sso.permission_set.template_generation import get_permission_set_dir
from iambic.core.utils import gather_templates


def attach_access_rule(
    permission_set_template: AWSSSOPermissionSetTemplate,
    aws_account: AWSAccount,
    exclude_accounts: int = 0,
) -> AWSSSOPermissionSetTemplate:
    users = [
        details["UserName"] for details in aws_account.sso_details.user_map.values()
    ]
    groups = [
        details["DisplayName"] for details in aws_account.sso_details.user_map.values()
    ]
    org_accounts = list(aws_account.sso_details.org_account_map.values())
    if exclude_accounts:
...
async def generate_permission_set_template_from_base(
    ...
    permission_sets = await gather_templates(
        repo_dir, AWS_SSO_PERMISSION_SET_TEMPLATE_TYPE
    )
    permission_set_dir = get_permission_set_dir(repo_dir)
    permission_set_template = AWSSSOPermissionSetTemplate.load(
