...
def templatize_resource(account_config: AccountConfig, resource):
    ...
    return json.loads(resource) if resource_type == dict or resource_type == list else resource_type(resource)
...
async def group_str_attribute(account_configs: list[AccountConfig], account_resources: list[dict]) -> Union[str | dict[str, list]]:
    """Groups a string attribute by a shared name across of accounts
    ...
    for account_resource_elem, account_resource in enumerate(account_resources):
        ...
        for resource_elem, resource in enumerate(account_resource["resources"]):
            account_resource["resources"][resource_elem]["account_id"] = account_resource["account_id"]
            resource_val = resource["resource_val"]
            templatized_resource_val = templatize_resource(account_config, resource_val)

            account_resources[account_resource_elem]["resource_val_map"][resource_val] = resource_elem
            account_resources[account_resource_elem]["elem_resource_val_map"][resource_elem] = [resource_val]
            if templatized_resource_val != resource_val:
                account_resources[account_resource_elem]["resource_val_map"][templatized_resource_val] = resource_elem
                account_resources[account_resource_elem]["elem_resource_val_map"][resource_elem].append(templatized_resource_val)

    grouped_resource_map = defaultdict(list)  # val:str = list(dict(name: str, path: str, account_id: str))
    # Iterate everything looking for shared names across accounts
    for outer_elem in range(len(account_resources)):
        for resource_val, outer_resource_elem in account_resources[outer_elem]["resource_val_map"].items():
            if outer_resource_elem is None:  # It hit on something already
                continue
            for inner_elem in range(outer_elem + 1, len(account_resources)):
                if (inner_resource_elem := account_resources[inner_elem]["resource_val_map"].get(resource_val)) is not None:
                    if not grouped_resource_map.get(resource_val):
                        # Null out the outer_resource_elem in all the places
                        for rn in account_resources[outer_elem]["elem_resource_val_map"][outer_resource_elem]:
                            account_resources[outer_elem]["resource_val_map"][rn] = None
                        account_resources[outer_elem]["elem_resource_val_map"][outer_resource_elem] = []

                        grouped_resource_map[resource_val] = [
                            account_resources[inner_elem]["resources"][inner_resource_elem],
                            account_resources[outer_elem]["resources"][outer_resource_elem],
                        ]
                    else:
                    ...
                    for rn in account_resources[inner_elem]["elem_resource_val_map"][inner_resource_elem]:
                        account_resources[inner_elem]["resource_val_map"][rn] = None
                    account_resources[inner_elem]["elem_resource_val_map"][inner_resource_elem] = []
    ...
    for account_resource in account_resources:
        for elem, resource_vals in account_resource["elem_resource_val_map"].items():
            ...
            account_resource["resources"][elem]["account_id"] = account_resource["account_id"]
...
async def group_dict_attribute(account_configs: list[AccountConfig], account_resources: list[dict]):
    ...
    for account_resource_elem, account_resource in enumerate(account_resources):
        ...
        for resource_elem, resource in enumerate(account_resource["resources"]):
            account_resource["resources"][resource_elem]["account_id"] = account_resource["account_id"]
            # Set raw dict
            resource_hash = xxhash.xxh32(json.dumps(resource["resource_val"])).hexdigest()
            hash_map[resource_hash] = resource["resource_val"]
            # Set dict with attempted interpolation
            templatized_dict = templatize_resource(account_config, resource["resource_val"])
            templatized_resource_hash = xxhash.xxh32(json.dumps(templatized_dict)).hexdigest()
            hash_map[templatized_resource_hash] = templatized_dict
            # Define resource hash mappings
            account_resources[account_resource_elem]["resource_hash_map"][resource_hash] = resource_elem
            account_resources[account_resource_elem]["elem_resource_hash_map"][resource_elem] = [resource_hash]
            if templatized_resource_hash != resource_hash:
                account_resources[account_resource_elem]["resource_hash_map"][templatized_resource_hash] = resource_elem
                account_resources[account_resource_elem]["elem_resource_hash_map"][resource_elem].append(templatized_resource_hash)

    grouped_resource_map = dict()  # val:str = list(dict(name: str, path: str, account_id: str))
    # Iterate everything looking for shared names across accounts
    for outer_elem in range(len(account_resources)):
        for resource_hash, outer_resource_elem in account_resources[outer_elem]["resource_hash_map"].items():
            if outer_resource_elem is None:  # It hit on something already
                continue
            for inner_elem in range(outer_elem + 1, len(account_resources)):
                if (inner_resource_elem := account_resources[inner_elem]["resource_hash_map"].get(resource_hash)) is not None:
                    if not grouped_resource_map.get(resource_hash):
                        ...
                        # Null out the outer_resource_elem in all the places
                        for rn in account_resources[outer_elem]["elem_resource_hash_map"][outer_resource_elem]:
                            account_resources[outer_elem]["resource_hash_map"][rn] = None
                        account_resources[outer_elem]["elem_resource_hash_map"][outer_resource_elem] = []
                    else:
                    ...
                    for rn in account_resources[inner_elem]["elem_resource_hash_map"][inner_resource_elem]:
                        account_resources[inner_elem]["resource_hash_map"][rn] = None
                    account_resources[inner_elem]["elem_resource_hash_map"][inner_resource_elem] = []
    ...
    for account_resource in account_resources:
        for elem, resource_hashes in account_resource["elem_resource_hash_map"].items():
            if not resource_hashes:
                ...
                # Take priority over raw output
                resource_hash = [rv for rv in resource_hashes if "{{" not in str(hash_map[rv])][0]
...
async def set_included_accounts_for_grouped_attribute(number_of_accounts_resource_on: int, grouped_attribute):

    if isinstance(grouped_attribute, dict):  # via group_str_attribute
        for k, resource_vals in grouped_attribute.items():
            included_accounts = [rv["account_id"] for rv in resource_vals]
        ...
        for elem in range(len(grouped_attribute)):
            if len(grouped_attribute[elem]["included_accounts"]) == number_of_accounts_resource_on:
