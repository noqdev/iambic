...
from noq_form.aws.iam.role.utils import list_roles, get_role_inline_policies, get_role_managed_policies
from noq_form.core.template_generation import templatize_resource, group_str_attribute, group_int_attribute, \
    group_dict_attribute, set_included_accounts_for_grouped_attribute
from noq_form.core.utils import NoqSemaphore, normalize_boto3_resp
from noq_form.core.logger import log

ROLE_RESPONSE_DIR = pathlib.Path.home().joinpath(".noqform", "resources", "aws", "roles")
...
def get_templated_role_file_path(role_name: str):
    ...
    file_name = role_name.replace("{{", "").replace("}}_", "_").replace("}}", "_").replace(".", "_").lower()
...
async def generate_account_role_resource_files(account_config: AccountConfig) -> dict:
    ...
    iam_client = account_config.get_boto3_session().client("iam", config=botocore.client.Config(max_pool_connections=50))
    ...
    for account_role in account_roles:
        role_path = os.path.join(account_role_response_dir, f'{account_role["RoleName"]}.json')
        ...
        messages.append(
            dict(
                file_path=role_path,
                content_as_dict=account_role,
                replace_file=True
            )
        )

    await role_resource_file_upsert_semaphore.process(messages)
    log.info("Finished caching AWS IAM Roles.", account_id=account_config.account_id, role_count=len(account_roles))

    return response


async def set_role_resource_inline_policies(role_name: str, role_resource_path: str, account_config: AccountConfig):
    iam_client = account_config.get_boto3_session().client("iam", config=botocore.client.Config(max_pool_connections=50))
    ...
    await role_resource_file_upsert(
        role_resource_path,
        {"InlinePolicies": role_inline_policies},
        False
    )


async def set_role_resource_managed_policies(role_name: str, role_resource_path: str, account_config: AccountConfig):
    iam_client = account_config.get_boto3_session().client("iam", config=botocore.client.Config(max_pool_connections=50))
    role_managed_policies = await get_role_managed_policies(role_name, iam_client)
    await role_resource_file_upsert(
        role_resource_path,
        {"ManagedPolicies": role_managed_policies},
        False
    )
...
async def set_templated_role_attributes(
    ...
    for role_ref in role_refs:
        async with aiofiles.open(role_ref["path"], mode='r') as f:
            content_dict = json.loads(await f.read())
            account_id_to_role_map[role_ref["account_id"]] = normalize_boto3_resp(content_dict)
    ...
    for account_id, role_dict in account_id_to_role_map.items():
        account_resources.append({"account_id": account_id, "resources": [{"resource_val": role_dict["path"]}]})
        managed_policy_resources.append({"account_id": account_id, "resources": [
            {"resource_val": mp["policy_arn"]} for mp in role_dict.get("managed_policies", [])
        ]})
        assume_role_policy_document_resources.append({"account_id": account_id, "resources": [
        ...
        if inline_policies := role_dict.get("inline_policies"):
            inline_policy_document_resources.append({"account_id": account_id, "resources": [
                {"resource_val": inline_policies}
            ]})

        if tags := role_dict.get("tags"):
            tag_resources.append({"account_id": account_id, "resources": [{"resource_val": tags}]})

        if description := role_dict.get("description"):
            description_resources.append({"account_id": account_id, "resources": [{"resource_val": description}]})

    description = None
    inline_policies = []
    tags = []
    assume_role_policy_documents = await group_dict_attribute(account_configs, assume_role_policy_document_resources)
    assume_role_policy_documents = await set_included_accounts_for_grouped_attribute(num_of_accounts, assume_role_policy_documents)
    ...
    if not isinstance(max_session_duration, int):
        ...
        for msd, account_ids in max_session_duration.items():
            max_session_duration_dicts.append({"included_accounts": account_ids, "max_session_duration": msd})
        max_session_duration = max_session_duration_dicts

    managed_policies = await group_str_attribute(account_configs, managed_policy_resources)
    # managed_policies = await set_included_accounts_for_grouped_attribute(num_of_accounts, managed_policies)

    if description_resources:
        description = await group_str_attribute(account_configs, description_resources)
        # if not isinstance(description, str):
        #     description_keys = await set_included_accounts_for_grouped_attribute(num_of_accounts, description)

    if tag_resources:
        tags = await group_dict_attribute(account_configs, tag_resources)
        tags = await set_included_accounts_for_grouped_attribute(num_of_accounts, tags)

    if inline_policy_document_resources:
        inline_policies = []
        inline_policy_vals = await group_dict_attribute(account_configs, inline_policy_document_resources)
        inline_policy_vals = await set_included_accounts_for_grouped_attribute(num_of_accounts, inline_policy_vals)
    ...
    if len(role_refs) == len(account_configs):
        ...
        included_accounts = [role_ref["account_id"] for role_ref in role_refs]
...
async def generate_aws_role_templates(configs: list[Config]):
    account_configs = []
    observed_account_ids = set()  # Ensure uniqueness on account_id in multi-config environment
    for config in configs:
        config.set_account_defaults()
        account_configs.extend(account_config for account_config in config.accounts if account_config.account_id not in observed_account_ids)
        observed_account_ids = set([account_config.account_id for account_config in account_configs])

    account_id_map = {account_config.account_id: account_config for account_config in account_configs}
    generate_account_role_resource_files_semaphore = NoqSemaphore(generate_account_role_resource_files, 5)
    set_role_resource_inline_policies_semaphore = NoqSemaphore(set_role_resource_inline_policies, 20)
    set_role_resource_managed_policies_semaphore = NoqSemaphore(set_role_resource_managed_policies, 30)
    log.info("Beginning to retrieve AWS IAM Roles.", accounts=list(account_id_map.keys()))
    ...
    for account_role_elem in range(len(account_roles)):
        for role_elem in range(len(account_roles[account_role_elem]["roles"])):
            ...
            account_roles[account_role_elem]["roles"][role_elem]["resource_val"] = role_name

        account_roles[account_role_elem]["resources"] = account_roles[account_role_elem].pop("roles", [])
