# AWS Template Models
* [RoleTemplate](schemas/role_template.md)
* [AssumeRolePolicyDocument](schemas/assume_role_policy_document.md)
* [ManagedPolicyRef](schemas/managed_policy_ref.md)
* [PolicyDocument](schemas/policy_document.md)
* [Path](schemas/path.md)
* [PermissionBoundary](schemas/permission_boundary.md)
* [MaxSessionDuration](schemas/max_session_duration.md)
* [RoleAccess](schemas/role_access.md)
* [PolicyStatement](schemas/policy_statement.md)

# Google Template Models
* [GroupTemplate](schemas/group_template.md)
* [GroupMember](schemas/group_member.md)

# Config Models
* [AWSAccount](schemas/awsaccount.md)
* [Config](schemas/config.md)
* [ExtendsConfig](schemas/extends_config.md)
* [Variable](schemas/variable.md)
