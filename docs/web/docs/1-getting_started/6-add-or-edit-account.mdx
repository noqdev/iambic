---
title: Adding or editing accounts in your IAMbic config
---
...
The exception to this is if the resource already exists on the account.
...
If another you have another IAM role template called `qa-intern` to be deployed in all accounts, and it does not exist on the new account, it will be created.

After templates are applied to the new account, IAMbic will run an import to pull in all resources unique to that account.
...
When you update an account through the wizard, in addition to updating your config, your templates will be synced.
