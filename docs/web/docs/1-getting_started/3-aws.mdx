...
12. Import existing Service Control Policies
13. Create a Policy
14. Attach a Policy to an AWS account, OU, or the root of your AWS Organization
...
In this section, we'll guide you through managing, creating, and attaching Service Control Policies (SCPs)
to your accounts, Organizational Units (OUs), or your entire organization. We'll also discuss how
to prevent drift in your SCPs.

Ensure that IAMbic is configured with the appropriate AWS account and region for your
AWS Organization. If correctly configured, you should see an `aws.organizations` section in
your `iambic_config.yaml` file.

If AWS Organizations is not yet configured, execute `iambic setup`, select `AWS`, then
`AWS Organizations`, and follow the prompts to configure IAMbic.

:::note
Please note that IAMbic only captures customer-managed SCPs. AWS-managed SCPs, such as
`FullAWSAccess` (The only one, to our knowledge), aren't captured by IAMbic.
:::

### 5.1 - Importing Existing Service Control Policies

Follow the steps below to import your existing SCPs:

1. Run the command `iambic import`.

Your existing customer-managed SCPs should now appear as IAMbic YAML templates under
`resources/aws/organizations/scp/ORG_ACCOUNT_NAME`. Here, `ORG_ACCOUNT_NAME` is the name of your
AWS Organization's management account.

### 5.2 - Creating a New Service Control Policy

This section provides a step-by-step guide on creating a new SCP. SCPs serve as governance
guardrails that can be applied across your organization. Since improper application can
potentially cause outages, it's important to thoroughly test SCPs before implementation.
For the purposes of this tutorial, we'll create SCPs that only contain `Allow` statements to prevent
accidental outages.

Replace the following placeholders in the template:

- `ORG_ACCOUNT_NAME`: Your AWS Organization's management account name.
- `ORG_ACCOUNT_ID`: Your AWS Organization's management account ID.
- `ORG_ID`: Your AWS Organization ID.
- `ORG_ROOT_ID`: The ID of your organization's root (You can find this in the AWS Console under `Organizations > Root`).

1. Create a new file for the SCP and save the template provided:
...
2. In the new file, specify the template type, identifier, and properties for the SCP.
The `iambic_managed: enforced` section will [prevent drift](/how_to_guides/prevent-drift) in a
production [GitOps/IAMOps](/reference/iamops_philosophy) setup.
...
3. Apply the changes using the IAMbic command-line interface:

```bash
iambic apply resources/aws/organizations/scp/ORG_ACCOUNT_NAME/iambic_test_scp.yaml
```

In a production setup, any SCPs that are added or modified will automatically be captured in Git
by IAMbic. This allows for a full audit trail of changes to your SCPs through Git History.
