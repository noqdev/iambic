...
export OKTA_ORG_NAME=<your_okta_org_name> # e.g.: development. This is a friendly name to identify your Okta organization.  Any arbitrary string will work here.
...
To delete the application, we can either set deleted: true or an expires_at
tag at the top level of the application, and then run `iambic apply resources/okta/app/$OKTA_IDP_NAME/test_application.yaml`:
...
The process for deleting a user is relatively similar. We can either set `deleted: true or an expires_at
