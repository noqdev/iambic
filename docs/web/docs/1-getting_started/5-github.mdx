...
1. Developer Alice wants to create an application role. She creates a different branch, makes your changes, and submits a pull request with her changes to the IAMbic templates repository.
...
4. Developer Alice updates the PR. The IAMbic Github action will run `iambic git-plan` again to calculate the new changes required.
...
3. Make sure you have admin access for the IAMbic Github repository that can perform the following tasks:
	...
	2. Create a fine-grained access token to allow automatic import of new entitlements.
