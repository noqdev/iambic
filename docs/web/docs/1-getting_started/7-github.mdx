...
a GitHub app, Amazon Eventbridge Rules, and an IAMbic Lambda function.
...
2. Once the PR is created, the IAMbic GitHub application will send a webhook event to the IAMbic lambda function to run
  `iambic plan --git-aware` to calculate the changes required due to the PR, and submits the proposed changes back into the PR as a comment.
3. Reviewer Bob will review the template changes along with the proposed_changes. Bob may request additional changes.
4. Optional: Developer Alice updates the PR. The IAMbic Github Application will send another Webhook event to re-run `iambic plan --git-aware` again to calculate the new changes.
5. Reviewer Bob approves the PR.
6. Developer Alice comments with `iambic apply` in the PR.
7. The IAMbic GitHub application will send another webhook event to the IAMbic lambda function to apply the changes from the PR into the cloud.
8. Once git-apply is successful, the Lambda function will merge the change into the default branch of the repository, and close the pull request.
...
1. Complete the [IAMbic Getting Started guide](/getting_started/) to create your Github repository and import your existing identities and
...
3. Make sure you have admin access for the IAMbic GitHub repository that can perform the following tasks:
	...
	2. Add GitHub Apps
4. Have AWS admin rights to deploy the IAMbic GitHub integration.
...
If you prefer to use `gh` (the [GitHub command line interface](https://cli.github.com/manual/)), here is the command:
...
1. Create the required IAM roles in the target AWS account.
1. Create the required ECR Pull Through Cache rule
1. Create the required ECR Repo to store your own copy of the `iambic` container image
1. Create the required CodeBuild project to pull the initial iambic image from the Public ECR Gallery to your own ECR repo
1. Create the required Lambda function and function url to receive webhook events for your GitHub app
