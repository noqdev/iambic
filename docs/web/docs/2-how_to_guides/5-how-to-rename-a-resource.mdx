...
To rename a resource, the recommended approach is to duplicate (copy) the resource file you'd like to rename, rename it with its new name, then set the `deleted: true` on the original resource template. See [How to delete a resource](/how_to_guides/how-do-i-delete-a-resource).
