...
Take, for example, the below examplar resource:
...
properties:
  ...
  tags:
    ...
    - key: noq-authorized-demo
      value: curtis@noq.dev:rohit@noq.dev
...
In order to apply, we run `iambic apply` - note that we can specify this template in particular to only apply the changes in this template file: `iambic apply -t resouces/aws/demo-1/demo1usertobedeleted.yaml` - you would need to change the path to the template after the `-t` flag to reflect your environemnt.
