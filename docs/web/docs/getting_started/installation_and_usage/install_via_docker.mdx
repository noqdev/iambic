...
Depending on where your cloud credentials are stored, you may have to mount the relevant directory inside the docker container as well:
    ...
    docker run -u $(id -u):$(id -g) -v ${HOME}/.aws:/root/.aws -v ${pwd}:/templates public.ecr.aws/s2p9s3r8/iambic:latest <command>
...
```bash
alias iambic="docker run -u $(id -u):$(id -g) -v <local_dir\>:/templates public.ecr.aws/s2p9s3r8/iambic:latest"
```
