...
#### Filtering Event Messages using AWS EventBridge Rule
To offer a tailored experience, the Event Rule that IAMbic has created could customize.
By modifying this rule, AWS CloudTrail messages can be filtered or selected specifically to suit your requirements.

To modify the AWS EventBridge rule go to [Amazon EventBridge page](https://us-east-1.console.aws.amazon.com/events/home?region=us-east-1#/rules),
select the rule that you want to modify and edit the `event pattern` attribute.
By default, IAMbic has the configuration listed below, what is listening to several events from IAM, SSO and Organizations.
...
An incorrect configuration might lead to omission of vital events or inclusion of undesired messages. Follow the guides below to understand more deeply how event patterns could be modified.
