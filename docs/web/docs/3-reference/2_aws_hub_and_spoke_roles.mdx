...
IambicHubRole on the Hub account and an IambicSpokeRole role on all the AWS accounts, which will be
assumed by the IambicHubRole to perform actions on those accounts.
